<x-app-layout>
    <div class="py-12" x-data="ordersManagement">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <h6 class="font-semibold text-2xl text-black dark:text-white mb-4">Orders</h6>

            <div class="mb-4">

                <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-4">
                    <div>
                        <x-select-input class="w-full capitalize" x-model="filterByPaymentStatus" @change="getOrders">
                            <option value="" selected>Payment Status</option>
                            <template x-for="status in ['pending', 'completed', 'failed', 'refunded']"
                                :key="status">
                    ...
                    <div>
                        <x-select-input class="w-full capitalize" x-model="filterByFulfillmentStatus"
                            @change="getOrders">
                            <option value="" selected>Fulfillment Status</option>
                            <template
                                x-for="categoryName in [
        ...
        <div class="w-full border-y border-slate-300 dark:border-slate-700 m:px-6 lg:px-8 bg-white dark:bg-slate-800">
                                ...
                                <td class="p-3">
                                    <div class="flex items-center flex-shrink-0 gap-3">
                                        <img :src="order.user.avatar" :alt="order.user.name"
                                            class="size-8 rounded-full object-cover object-center">

                                        <span x-text="order.user.name"></span>
                                    </div>
                                </td>
            ...
            <div
                ...
                <div class="flex  items-center space-x-2">
                    ...
                    <x-select-input id="rowsPerPage" x-model="rowsPerPage" @change="getOrders">
    ...
    @push('scripts')
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('ordersManagement', function() {
                    return {
                        orders: {},
                        isLoading: true,

                        // pagination
                        rowsPerPage: this.$persist(10),
                        from: 1,
                        to: 10,
                        total: 0,
                        currentPage: 1,
                            ...
                            try {
                                ...
                                const response = await axios.get('{{ route('admin.orders.list') }}', {
                                    params: {
                                        ...
                                        filterByPaymentStatus: this.filterByPaymentStatus,
                                        filterByFulfillmentStatus: this.filterByFulfillmentStatus,
